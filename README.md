...
订单详情提供零成本的物流轨迹跳转：在「物流轨迹」一栏点击即跳转到快递100网页版查询页（新窗口打开，单号自动带入，已识别的快递公司自动预选，未收录的公司按单号智能识别）。该跳转不调用任何付费 API，仅打开公共查询网页；顺丰等需要手机号校验的快递由查询页自行处理。
