...
采购订单录入页支持智能识图：上传一张平台订单详情截图（京东 / 拼多多 / 淘宝 / 唯品会 / 抖音），系统调用视觉大模型抽取采购渠道、平台订单号、快递公司、快递单号以及每个商品的名称 / 货号 / 尺码 / 数量 / 实付金额，自动填入表单并把这张截图加入订单附件；识别结果只是草稿，提交前请核对。需要在服务器配置 `VISION_*` 环境变量（见下文）。每次进入「新增 / 上报」都会清空上一单留下的表单内容；从订单列表点编辑才会带出原单。
...
- `VISION_API_BASE`、`VISION_API_KEY`、`VISION_MODEL`：可选智能识图服务，用于上传订单截图自动填写采购订单。走 OpenAI 兼容的 `chat/completions` 协议，通义千问 VL（`https://dashscope.aliyuncs.com/compatible-mode/v1` + `qwen-vl-plus`）、豆包、智谱 GLM-4V、GPT-4o 等均可直接使用；`VISION_TIMEOUT_MS` 为单次识别超时（默认 45000）。未配置时识图入口会提示"尚未配置"，其余功能不受影响
