...
完整的业务规则、技术实现和维护指南见 [系统知识库](docs/系统知识库.md)。当前库表、字段、索引和外键见 [数据库设计](docs/数据库设计.md)。采购员与管理员的录单分步操作见 [上传订单教程](public/tutorial/upload-order.html)（采购员在应用首页「操作教程」打开的也是这份）。
...
管理员可在订单列表直接导出 Excel：勾选订单时导出已选订单，未勾选时导出当前筛选结果；文件按商品明细逐行列出商品名、尺码、件数、采购运单号和平台订单号。旁边的「导出文案」按同样范围生成可直接粘贴的采购清单：首行是导出时间（`MMDD HH:mm:ss`，服务器时区），然后是「货号 商品名 尺码规格 数量」表头和按货号、商品名、尺码合并件数后的商品行（同一款的不同尺码排在一起），末尾是「合计：N件」；优先复制到剪贴板，剪贴板不可用时改为下载 txt。成员页仍保留完整字段的 CSV 导出。
...
订单详情的商品清单里，每个商品行右侧显示整行实付金额；数量大于 1 的商品行会在总金额下方用小字显示「单价」（总金额 ÷ 数量，保留两位小数）。
...
- `VISION_API_BASE`、`VISION_API_KEY`、`VISION_MODEL`：可选智能识图服务，同时用于采购订单截图自动填写、管理员拍照识别快递面单和结款截图金额识别。当前使用智谱 OpenAI 兼容接口（`https://open.bigmodel.cn/api/paas/v4` + `glm-4.6v`）；API Key 仅保存在本机或服务器环境变量，不进入代码和镜像。订单截图中若规格同时包含尺码和颜色/款式描述，系统会保留纯尺码，并在没有明确货号时优先用其余规格描述作为货号。面单识别后会按运单号反查采购单（匹配订单头和商品行快递单号）。`VISION_TIMEOUT_MS` 为单次识别超时（未设置时单张默认 45000，多张默认 75000，部署示例统一设为 75000）。未配置时识图入口会提示"尚未配置"，其余功能不受影响
