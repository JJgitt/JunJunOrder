...
流程：推送 → 测试/数据库迁移验证 → main 构建镜像 → GHCR（必推）→ 同摘要镜像到华为云 SWR 和阿里云 ACR（各自失败不阻断）→ 选仓 → SSH 拉取摘要镜像 → 数据库备份 → 切换 → 健康检查。

选仓顺序：`IMAGE_REGISTRY=swr` 时先 SWR，SWR 登录或同步失败则用阿里云 ACR，两者都不可用才落回 GHCR（服务器跨境拉 GHCR 可能很慢，是最后一档）。`IMAGE_REGISTRY=acr` 时顺序为 ACR → SWR → GHCR。三处凭证只放在 GitHub Secrets，仓库里保留 2026-09-15 的 GHCR 快照：`deploy/ci/rollback/ghcr-20260915/`。
...
- GitHub 并发组 + 服务器 flock 双重串行保护。测试失败不会发布；首选仓库登录或镜像同步失败时，按上面的顺序自动改拉本次构建的同摘要镜像。每次镜像拉取最多等待 5 分钟、最多尝试 3 次，全部失败也不会切换线上应用。
...
| SWR_USERNAME | 华为云 SWR 登录用户，例如 `cn-north-4@AK`（用长期有效登录指令，临时指令 24 小时过期） |
...
| IMAGE_REGISTRY | 空或 `ghcr`：服务器只拉 GHCR；`swr`：SWR → ACR → GHCR；`acr`：ACR → SWR → GHCR（服务器脚本须已包含对应白名单） |

推送镜像用 GitHub 自动提供的 `GITHUB_TOKEN`（publish 作业 packages:write）；部署作业仅 packages:read。短期令牌通过 SSH 标准输入传入，在临时 Docker 配置目录中使用，结束后删除。无需长期 GHCR PAT。SWR / ACR 镜像都是 GHCR 构建结果的同摘要副本；登录或同步失败时不阻断发布，流水线按顺序换仓并留下警告。
...
目标仓库：`crpi-lz061y1f8ajv9wzf.cn-guangzhou.personal.cr.aliyuncs.com/junjunorder/junjunorder`（个人版实例，华南·广州，私有）。个人版免费、无 SLA、单账号一个实例，这里只作 SWR 不可用时的兜底。

启用顺序（服务器要先认识 ACR 地址，否则被选中时会被服务器拒绝、本次发布失败但线上不受影响）：

1. 推送本仓库后，在服务器用 root 执行 `bash /home/junjun/hongyun-order/deploy/ci/install-server-deploy.sh`。
...
3. `IMAGE_REGISTRY` 保持 `swr`：正常走 SWR，SWR 抽风时自动走 ACR。想以 ACR 为主就设为 `acr`。

停用：删掉 `ACR_REPOSITORY` 变量即可，流水线会跳过 ACR 的登录与同步。
