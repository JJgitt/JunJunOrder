...
function AppHeader({ user, page }: { user:AppUser; page:string }) {
  const labels: Record<string,string> = { dashboard:"管理看板", stock:"库存管理", orders:"订单管理", profile:"我的", home:"骏骏订单", upload:"上传订单", mine:"我的订单" };
...
function AdminDashboard({ orders, stock, onReceipt, onOrders, onStock }: { orders:PurchaseOrder[]; stock:StockItem[]; onReceipt:() => void; onOrders:() => void; onStock:() => void }) {
...
function AdminOrders({ orders,onOpen,onApprove,onReject,onShip }: { orders:PurchaseOrder[];onOpen:(id:string)=>void;onApprove:(id:string)=>void;onReject:(id:string)=>void;onShip:(id:string)=>void }) {
...
function UploadPage({editing,onSubmit}:{editing?:PurchaseOrder;onSubmit:(order:PurchaseOrder,files:File[])=>Promise<void>}){
  ...
  return <section className="enter upload-page"><div className="form-intro"><span>{editing?"驳回订单修改":"NEW PURCHASE"}</span><h2>{editing?"修改并重新提交":"上报采购订单"}</h2><p>订单与附件将安全保存到云端，提交后可跨设备查看。</p></div>{editing?.rejectReason&&<div className="inline-warning"><b>驳回原因</b><span>{editing.rejectReason}</span></div>}<form className="purchase-form" onSubmit={submit}><div className="field-grid"><label><span>采购渠道 *</span><select value={platform} onChange={e=>setPlatform(e.target.value)}><option>京东</option><option>拼多多</option><option>淘宝</option><option>唯品会</option><option>其他</option></select></label><label><span>平台订单号 *</span><input value={platformNo} onChange={e=>setPlatformNo(e.target.value)} placeholder="请输入订单号"/></label></div><label><span>商品名称 *</span><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="如 乔丹 DUNK LOW 熊猫"/></label><div className="field-grid"><label><span>货号 *</span><input value={sku} onChange={e=>setSku(e.target.value)} placeholder="DD1391-100"/></label><label><span>尺码 *</span><input value={size} onChange={e=>setSize(e.target.value)} placeholder="42 / 41.5"/></label></div><div className="field-grid"><label><span>数量 *</span><input type="number" min="1" value={qty} onChange={e=>setQty(Number(e.target.value))}/></label><label><span>实付金额 *</span><div className="money-input"><i>¥</i><input inputMode="decimal" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0.00"/></div></label></div><label><span>快递单号 <em>建议必填</em></span><input value={courier} onChange={e=>setCourier(e.target.value)} placeholder="粘贴物流单号，方便仓库识别收货"/></label><label className="upload-zone"><input type="file" accept="image/*" multiple onChange={e=>setFiles(Array.from(e.target.files??[]).slice(0,3))}/><i>＋</i><b>{files.length?`已选择 ${files.length} 张截图`:"上传订单截图"}</b><span>持久化保存，最多 3 张、单张不超过 5MB</span></label><button className="primary-button" disabled={submitting} type="submit">{submitting?"正在保存…":editing?"重新提交审核":"提交订单"}</button><p className="form-footnote">平台与订单号组合唯一，系统会自动阻止重复上报</p></form></section>;
}
