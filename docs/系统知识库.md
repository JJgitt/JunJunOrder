...
- 订单详情弹层提供「物流轨迹」跳转入口：按商品行的采购快递单号生成快递100网页版查询链接（`lib/courier.ts` 的 `courierTrackingUrl`，已知快递公司携带 `com` 参数，其他公司仅传单号由页面智能识别），新窗口打开，不调用任何付费 API。
...
- `lib/tracking.ts` 封装快递鸟「即时查询」：ShipperCode 映射、MD5+Base64+URL 编码签名、超时与 10 分钟进程内缓存；由 `/api/tracking` 独立 Route Handler 暴露。
