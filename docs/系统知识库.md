...
- 订单详情弹层提供「物流轨迹」跳转入口：按商品行的采购快递单号生成网页查询链接（`lib/courier.ts` 的 `courierTrackingUrl`）。已知快递公司打开快递鸟对外免费的移动端结果页 `JSInvoke/MSearchResult.aspx?expCode=&expNo=&backUrl=`（服务端直出轨迹，返回键回到本系统）；未收录的公司打开快递100 网页版按单号识别。新窗口打开，纯跳转，不调用任何付费 API，服务器无需物流凭证。
